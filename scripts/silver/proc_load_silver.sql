...
-- Expectations: No Results
    PERFORM cst_firstname
    FROM bronze.crm_cust_info
    WHERE cst_firstname != TRIM(cst_firstname);

-- Data Standardization & Consistency
    PERFORM DISTINCT prd_line
...
-- Check for unwanted Spaces
    ...
    FROM silver.crm_cust_info
    WHERE cst_firstname != TRIM(cst_firstname);
    PERFORM cst_lastname
    FROM silver.crm_cust_info
    WHERE cst_lastname != TRIM(cst_lastname);

-- Data Standardization & Consistency
    PERFORM DISTINCT cst_gndr
    FROM silver.crm_cust_info;
...
--- Data Overview ---
    PERFORM prd_id, prd_key, prd_nm, prd_cost, prd_line, prd_start_dt, prd_end_dt
    from bronze.crm_prd_info;
...
-- Identify out-of-range date values
    ...
    PERFORM DISTINCT erp_dt
    FROM silver.erp_cust
    WHERE erp_dt < '1924-01-01'
       OR erp_dt > NOW();

-- Data Standardization & Consistency
    PERFORM DISTINCT erp_gndr,
                    CASE
                        WHEN UPPER(TRIM(erp_gndr)) IN ('F', 'FEMALE') THEN 'Female'
                        WHEN UPPER(TRIM(erp_gndr)) IN ('M', 'MALE') THEN 'Male'
                        ELSE 'n/a' END as erp_gndr
...
-- Data Standardization & Consistency
    PERFORM DISTINCT erp_px_cat.erp_cat FROM bronze.erp_px_cat;
